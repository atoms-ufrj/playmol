...
contains
  ...
  subroutine packmol_xyz_files( lpack, lmol, lcoord, natoms, molfile, print )
    ...
    integer :: i, n, ifile, nfiles, imol, unit, iatom
    type(Struc), pointer :: ptr, coord
    integer, allocatable :: molecule(:), position(:)
    ...
    do ifile = 1, nfiles
      ...
      do while (associated(ptr).and.(i < n))
        if (str2int(ptr % params) == imol) then
          i = i + 1
          position(i) = lcoord % index( ptr % id )
        end if
        ptr => ptr % next
      end do
      if (any(position(1:n) == 0)) call error( "no coordinates for molecule", int2str(imol) )

      open( newunit = unit, file = molfile(ifile), status = "replace" )
      write(unit,'(A)') trim(int2str(natoms(imol)))
      write(unit,'("# Generated by playmol")')
      coord => lcoord % first
      do i = 2, minval(position(1:n))
        coord => coord % next
      end do
