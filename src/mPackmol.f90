...
contains
  ...
  subroutine tPackmol_run( me, mol, latom, ldiam, Lbox )
    use iso_fortran_env, only : screen => output_unit
    class(tPackmol), intent(inout) :: me
    type(tMolecule), intent(inout) :: mol
    type(StrucList), intent(inout) :: latom, ldiam
    ...
    contains
      ...
      subroutine retrieve_coordinates( mixfile )
        ...
        call mol % xyz % destroy( silent = .true. )
        open( newunit = mix, file = mixfile, status = "old" )
        read(mix,'(I12,/)') natoms
        allocate( data(natoms,4), ndata(natoms) )
        ndata = 4
        do iatom = 1, natoms
          read(mix,'(A'//csl//')') line
